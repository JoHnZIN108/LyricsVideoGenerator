...
THEME = os.environ.get("THEME", "neon")  # neon (Neon Blueprint) | instrument | patent (igotchu Type Lab styles)
...
KEYBG = {"neon": "#18265a", "instrument": "#ffffff", "patent": "#f7faf8"}.get(os.environ.get("THEME", "neon"), "#18265a")
...
for i, c in enumerate(typed):
    ...
    if kid:
        raw(f'tl.fromTo("{kid}",{{backgroundColor:"#ff7a1a",scale:1.25}},{{backgroundColor:"{KEYBG}",scale:1,duration:.25,ease:"power1.out",immediateRender:false}},{t});')
...
# ================================================================ THEMES (igotchu Type Lab: Instrument, Patent)
TYPELAB_FONTS = (
...
""",
  "decor": lambda n_slides: (
      ...
      '<div class="silk" style="left:96px;top:44px">igotchu / Ep 03 — ChatGPT is just guessing</div>'
...
THEME_CSS, DECOR, EASES, EXTRA_FONTS = "", "", ("0.16,1,0.3,1", "0.34,1.56,0.64,1", "0.7,0,0.84,0", "0.65,0,0.35,1"), ""
if THEME != "neon":
    th = THEMES[THEME]
    THEME_CSS, DECOR, EASES, EXTRA_FONTS = th["css"], th["decor"](10), th["eases"], TYPELAB_FONTS
    for n in range(1, 11):  # per-slide tag (and figure number) swaps with the slide
        for pre in ("tag", "fig"):
    ...
    if os.path.isdir(OUT):
        shutil.rmtree(OUT)
    shutil.copytree("video", OUT, ignore=shutil.ignore_patterns("index.html", "mix.mp3", "*.mp4"))
    for f in ["@fontsource-variable/bricolage-grotesque/files/bricolage-grotesque-latin-wdth-normal.woff2"] + \
             [f"@fontsource/dm-sans/files/dm-sans-latin-{w}-normal.woff2" for w in (500, 600, 700)] + \
             [f"@fontsource/jetbrains-mono/files/jetbrains-mono-latin-{w}-normal.woff2" for w in (500, 700)]:
        shutil.copy(f"../node_modules/{f}", f"{OUT}/assets/fonts/")

html = f"""<!doctype html>
<html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=1920, height=1080"/>
<script src="assets/gsap.min.js"></script><script src="assets/CustomEase.min.js"></script>
<style>{FONTS}{EXTRA_FONTS}{tokens_css()}{bundle_css()}{LOCAL}{THEME_CSS}</style></head>
<body>
...
subprocess.run(["ffmpeg", "-y", "-v", "error", "-i", "sfx/_mix.wav", "-af", "loudnorm=I=-16:TP=-1.5:LRA=11", "-ar", "44100", "-ac", "2", "-b:a", "192k", f"{OUT}/assets/mix.mp3"], check=True)
